...
$join = [
    ...
    'LEFT JOIN ' . db_prefix() . 'taggables ON ' . db_prefix() . 'tasks.id = ' . db_prefix() . 'taggables.rel_id',
...
array_push($where, 'AND ' . db_prefix() . 'taggables.rel_type = "task"');
...
$additionalColumns = hooks()->apply_filters('scorecards_table_additional_columns_sql', [
    count(db_prefix() . 'tasks.id'),
    db_prefix() . 'staff.firstname',
    db_prefix() . 'staff.lastname',
]);

$sGroupBy = ' GROUP BY ' .db_prefix() . 'clients.company, ' .db_prefix() . 'tags.name, ' .db_prefix() . 'projects.name, '. db_prefix() . 'staff.firstname, ' . db_prefix() . 'staff.lastname' ;
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        elseif ($aColumns[$i] == db_prefix() . 'tasks.id') {
            $_data = count($aRow['firstname']) ;
        }
